...
After root `gzh-design` creates the preview, run `scripts/upgrade_preview_copy.py output/article-preview.html`. This preserves the original copy fallback but writes the intended fragment as explicit `text/html` when the browser supports the Clipboard API, preventing Chromium from flattening block hierarchy during selection-based copy. A missing or unrecognized `gzhCopy` function blocks the gate; do not silently claim that the preview was upgraded.
...
Do not mark HTML complete unless `gzh-design`, deterministic validation, both browser preview checks, and the pasted-DOM inspection have all passed. Missing `gzh-design` blocks HTML completion; no generic formatter or hand-written HTML substitutes for this gate.
