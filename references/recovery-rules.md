...
| Evidence, source, factual support, or `research/evidence.md` / `research/sources.json` changes | `evidence` | Recheck facts, then rebuild affected outline, text, prediction, visuals, HTML, publish, and retro work. |
...
| Any content in `drafts/final.md` changes, including wording-only edits | `final` | Create a new immutable Cheat prediction version, then recheck image positions and regenerate HTML. |
| `visuals/visual-plan.md` changes | `visual_plan` | Regenerate affected visual assets and every downstream artifact. |
| A visual asset changes without changing text or the visual plan | `visuals` | Preserve text and prediction; regenerate and validate HTML and later delivery records. |
...
- A confirmed topic angle or topic-strategy change calls `invalidate_from(state, "topic")` and invalidates evidence, outline, drafts, prediction reference, visual plan, visuals, HTML, publish, and retro.
- Fact or structural change in final text invalidates the fact gate, prediction, visual plan, visuals, HTML, publish, and retro. Recovery must continue through every previously completed downstream stage; it cannot stop after regenerating HTML.
- Wording-only final-text change is still a final-content change: call `invalidate_from(state, "final")`, review hashes and image positions, obtain a newly versioned prediction, and regenerate HTML. Any completed publish and retro work also becomes stale. Do not reuse an old prediction merely because the meaning appears unchanged, and do not stop recovery at HTML.
- Image-only change preserves text and prediction; call `invalidate_from(state, "visuals")` and rerun HTML layout and validation. Any completed publish and retro work becomes stale and must be recovered after the new HTML is approved.
- Visual-plan change calls `invalidate_from(state, "visual_plan")`; visuals, HTML, completed publish work, and completed retro work become stale. Do not treat it as an image-only replacement or stop recovery at HTML.
...
`invalidate_from` marks completed downstream work and recorded downstream artifacts stale while preserving incomplete downstream statuses that have no artifact. It clears approvals keyed to the changed or a downstream stage and approvals bound to artifacts in that range, while preserving earlier unaffected approvals. After rebuilding one role, call `record_artifact` for that role. Re-recording different bytes clears every approval bound to that role; re-recording the same SHA256 preserves them. Re-recording also clears only that role's stale artifact marker and marks that role completed; it does not clear or validate any unrecovered downstream role. The current final artifact must be approved again before prediction, and the approval hash must equal the current final artifact hash. The orchestrator may re-record `prediction`, thereby clearing `create_new_prediction_version`, only after the mandatory Cheat call has produced and preserved the new external version. Continue recovery until every affected downstream stale marker has been regenerated and validated.
...
- Missing `gzh-design`: block HTML generation, validation, preview completion, and HTML delivery. There is no generic fallback route for the HTML gate.
- 21:9 cover or body-illustration failure: preserve the prompt and failure record; do not mark visual delivery complete.
- HTML failure: preserve valid upstream artifacts and block HTML completion.
